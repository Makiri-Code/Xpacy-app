...
const Card = ({propertise,  cardStyles}) => {
    ...
    const {cardWidth, cardHeight, titleSize, headingSize, iconWidth, iconHeight, imgHeight, likeIconSize, bodyGap, bodyPadding, headerGap, showDivider, priceSize, showButtons, isMobile} = cardStyles
    const {images, property_name, property_status, city, state, property_price, total_bedrooms, id} = propertise;
    
    const fetchProperty = async (id) => {
        try {
            const property = await fetch(`https://app.xpacy.com/property/fetch-property/${id}`, {method: 'GET'})
    ...
    return (
        <div className="propertise-card d-flex flex-column align-items-start" style={{width: `${isMobile ? '313px': cardWidth}` , height: cardHeight,}} onClick={handleClick}>
            <div className="card-img-container position-relative align-self-stretch">
                <img className="card-image" src={images[0]} alt="house image" style={{height: imgHeight}}/>
                <div className="card-image-btns d-flex justify-content-between align-items-center position-absolute">
                    <div className="btn rounded-pill d-flex justify-content-center align-items-center">{property_status.charAt(0).toUpperCase() + property_status.slice(1).toLowerCase()}</div>
                    <div className="heart-icon-container d-flex justify-content-center align-items-center" style={{width: likeIconSize, height: likeIconSize}}>
                        {
                            showLikeBtn ? <FaHeart className="heart-icon" onClick={() => setShowLikeBtn(!showLikeBtn)}/> : <FaRegHeart className="heart-icon" onClick={() => setShowLikeBtn(!showLikeBtn)}/>
                        }
                    </div>
                </div>
            </div>
